...
def get_prompts(dataset, args):
    if dataset == 'pickapic':
        ...
        if args.reward_fn =="aesthetic_score":
            return all_nouns()
        return all_nouns_activities(generate_negative_samples=args.generate_negative_samples)
...
def all_nouns_activities(nouns_file='assets/simple_animals.txt', activities_file='assets/activities.txt', generate_negative_samples=False):
    ...
    if generate_negative_samples:
        list_all_negative_prompts = {"no_noun": [], "no_activity": []}
        
        for noun in nouns:
            list_all_negative_prompts["no_activity"].append(f"{IE.a(noun)}")
            
        for activity in activities:
            list_all_negative_prompts["no_noun"].append(f"{activity}") 
            
    if generate_negative_samples:
        return list_all_prompts, list_all_negative_prompts
    else:
